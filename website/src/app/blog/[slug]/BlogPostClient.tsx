...
export default function BlogPostDetail({ params, initialPost, initialRelatedPosts, initialCategories }: BlogPostDetailProps) {
  const { slug } = params;

  const [currentPost, setCurrentPost] = useState<BlogPost | null>(initialPost ?? null);
  const [relatedPosts, setRelatedPosts] = useState<BlogPost[]>(initialRelatedPosts ?? []);
  const [blogCategories, setBlogCategories] = useState<BlogCategory[]>(initialCategories ?? []);
